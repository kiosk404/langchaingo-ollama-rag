...
func getOllamaLlama2() *ollama.LLM {
	// 创建一个新的ollama模型，模型名为"llama2-chinese"
	llm, err := ollama.New(
		ollama.WithModel("llama2-chinese"),
		ollama.WithServerURL(ollamaServer))
	if err != nil {
		logrus.Fatal("创建ollama模型失败: %v", err)
	}
...
func getStore() *qdrant.Store {
	...
	if err != nil {
		logrus.Fatal("创建qdrant存储失败: %v", err)
	}
	return &store
}
